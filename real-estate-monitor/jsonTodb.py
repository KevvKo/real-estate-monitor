...
for entry in json:
    ...
    values = (entry['domain'], int(entry['expose']), entry['date'],entry['coldrent'], entry['roomnumber'], entry['surface'], entry['sidecosts'], entry['street'], str(entry['postcode']), entry['town'], entry['latitude'], entry['longitude'], entry['hash']) 
